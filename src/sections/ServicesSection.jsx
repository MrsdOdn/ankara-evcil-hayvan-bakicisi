...
const ServicesSection = () => {
  const [services, setServices] = useState([
    {
      ...
      dbKey: "gezdirmeDesc", // Firebase'deki anahtar ile eşleşti
      color: "bg-orange-500",
      icon: (
    ...
    },
    {
      ...
      dbKey: "kuaforDesc", // Firebase'deki anahtar ile eşleşti
      color: "bg-blue-500",
      icon: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      dbKey: "sosyalDesc", // JSON'da 'bakimDesc' yoktu, görseldeki 'sosyalDesc'e bağlandı
      color: "bg-green-500",
      icon: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      dbKey: "sosyalDesc", // Firebase'deki anahtar ile eşleşti
      color: "bg-purple-500",
      icon: (
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(dbRef, (snapshot) => {
      ...
      if (data) {
        // Başlıkları Firebase'deki tam isimlerle güncelliyoruz
        setTitles({
          ust: data.hizmetlerUstBaslik || "Neler Yapıyoruz?",
          ana: data.hizmetlerAnaBaslik || "Dostlarınız İçin Tam Kapsamlı Bakım"
        });

        // Servisleri Firebase'den gelen desc'lere göre eşleştiriyoruz
        setServices(prev => prev.map(s => ({
          ...s,
          description: data[s.dbKey] || s.description
        })));
      }
    });
    return () => unsubscribe();
